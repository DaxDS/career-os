...
class ImmigrationScoringAgent(LLMAgentBase):
    def score(
        self,
        user_id: uuid.UUID,
        job: JobPosting,
        ...
        result, _ = self._invoke(
            user_id=user_id,
            job_id=job.id,
            agent_name="immigration_scoring",
            capability=AICapability.IMMIGRATION_SCORING,
            prompt_name=PromptName.IMMIGRATION_SCORING,
            variables={
                "job_title": job.title,
                "company": job.company,
